...
export default {
  ...
  methods: {
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/marca/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/marca", this.selectedItem)
          }
    ...
    async nameValidate(){
      ...
      let res = await axios.get('api/marca/fill/'+JSON.stringify(params))
