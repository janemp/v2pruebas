...
export default {
  ...
  methods: {
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/provincia/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/provincia", this.selectedItem)
          }
    ...
    async codevalidate(){
      ...
      let res = await axios.get('api/provincia/show_fill/'+JSON.stringify(params))
