          ...
          <v-stepper-items>
              <v-stepper-content step="1">
                <v-form ref="form1" v-model="valid" lazy-validation>
                  ...
                  <v-layout row wrap>
                    <v-flex xs6>
                      <v-text-field
                        ...
                        label="Númer de identificación tributaria (NIT)"
                    ...
                    <v-flex xs6>
                      <v-text-field
                        v-model="selectedEmpresa.numero_fundempresa"
                        label="Número de fundempresa"
                        :rules="[v => !!v || 'Requerido', v => (v && v.length <= 15) || 'No mayor a 15 caracteres']"
                        hint="<span class='blue--text'>*Requerido</span>" persistent-hint                        
                      ></v-text-field>
                    </v-flex>
                  ...
                  <h3 align="center">REPRESENTANTE LEGAL</h3>
                  <v-layout row wrap>
                    <v-flex xs4>
                      <v-text-field
...
export default {
  ...
  mounted() {
    this.bus.$on("openDialog", item => { 
      ...
      this.selectedItem = item.persona
      this.pais_nacimiento = item.persona.pais_nacimiento.nombre
      this.birth_date_formatted = this.$moment(item.fecha_nacimiento).format("DD/MM/YYYY")
      this.img = item.persona.fotografia
  ...
  methods: {    
    ...
    async save() {
      try {
        ...
        this.selectedEmpresa.user_id = this.$store.getters["auth/user"].id
        if (this.$refs.form2.validate()) {
          ...
          if (this.selectedIndex != -1) {            
            ...
            await axios.put("api/empresa/"+this.selectedEmpresa.id, this.selectedEmpresa)
          } else {
            this.selectedItem.tipo_persona_id = 5          
            let res = await axios.post("api/persona", this.selectedItem)
            this.selectedEmpresa.persona_id = res.data.id
            await axios.post("api/empresa", this.selectedEmpresa)
          }
