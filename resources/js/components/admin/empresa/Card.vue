      ...
      <v-card-text>        
        <v-data-table
          ...
          <template slot="items" slot-scope="props">
            <tr>
              <td class="text-xs-center" @click="props.expanded = !props.expanded"> {{ props.item.autorizacion }} </td>
              ...
              <td class="text-xs-center" @click="props.expanded = !props.expanded">
                <v-chip v-if="$moment(props.item.fecha_conclusion).format('DD/MM/YYYY') >= $moment().format('DD/MM/YYYY')" small color="green" text-color="white">vigente</v-chip>
                <v-chip v-else small color="orange" text-color="white">vencido</v-chip>
              </td>
...
export default {
  ...
  mounted() {
    this.bus.$on("openDialogCard", item => {       
      ...
      this.getTable();
      if(item.autorizaciones.length > 0) {
  ...
  methods: {    
    ...
    async getTable() {
      try {
        let res = await axios.get("api/autorizacion/fill/" + JSON.stringify({empresa_id: this.personaId}))
        this.table = res.data
    ...
    async removeItem(item) {
      this.bus.$emit("openDialogRemove", `api/autorizacion/${item.id}`);
    },
    fullName(item) {
    ...
    async printItem(item) {
      try {        
        let res = await axios({
          method: "GET",
          url: "api/autorizacion/print/" + item.id,
