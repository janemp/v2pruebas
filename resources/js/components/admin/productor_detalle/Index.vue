<template>
  <v-container fluid>
    <v-toolbar>
        <v-toolbar-title>Productores al detalle</v-toolbar-title>
        <v-spacer></v-spacer>   
        <v-divider
          class="mx-2"
          inset
...
export default {
  ...
  methods: {
    async getTable() {
      try {
        let res = await axios.get("api/persona/fill/" + JSON.stringify({'tipo_persona_id': 2}))
        this.table = res.data
      } catch (e) {
        console.log(e)
      }
    ...
    async removeItem(item) {
      ...
      let res2 = await axios.get("api/hoja_ruta/fill/" + JSON.stringify({persona_id: item.id}));
      if(res.data.length > 0 || res2.data.length > 0) {
        this.$toast.info('Este productor tiene hoja de ruta o guía de internación.')
      } else {
