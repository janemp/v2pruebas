<template>
  <v-dialog persistent v-model="dialog" max-width="900px" @keydown.esc="close">    
    <v-card>
      ...
      <v-toolbar>
        <v-toolbar-title>Decomisos</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-divider
          class="mx-2"
          inset
...
export default {
  ...
  mounted() {
    this.bus.$on("openDialogDecomisar", item => {
      this.dialog = true      
      this.selectedItem = item
      this.getTable();
    });    
    this.bus.$on("closeDialogDecomiso", () => {
      this.getTable();
    });
  },
  computed: {
  ...
  methods: {    
    ...
    async getTable() {
      try {
        let res = await axios.get("api/decomiso")
        this.table = res.data
      } catch (e) {
        console.log(e)
      }
    ...
    nuevoItem(item) {
      this.bus.$emit("openDialogDecomisoForm", item);
    },
    async removeItem(item) {
      this.bus.$emit("openDialogRemove", `api/decomiso/${item.id}`)
    },
    fullName(item) {
