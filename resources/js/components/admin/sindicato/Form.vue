            ...
            <v-flex xs12 sm6 md6>
              <v-card-text>
                <v-select
                  ...
                  item-text="nombre"
                  item-value="id"
                  v-model="selectedItem.central_id"
                  label="Central"
                  :rules="[v => !!v || 'Requerido']"
...
export default {
  ...
  methods: {
    async getCentrales() {
      let res = await axios.get("api/central")
      this.centrales = res.data
    },
    close() {
      this.dialog = false;
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/sindicato/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/sindicato", this.selectedItem)
          }
    ...
    codevalidate(){
      axios.get('/api/sindicato/codigo/'+this.selectedItem.codigo).then(response =>{
