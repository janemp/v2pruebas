<template>
  <v-container fluid>
    <v-toolbar>
        <v-toolbar-title>Sindicatos</v-toolbar-title>
        <v-spacer></v-spacer>        
        <v-divider
          class="mx-2"
          inset
    ...
    <v-data-table
        ...
        <template slot="items" slot-scope="props">
          <tr>
            <td class="text-xs-center"> {{ props.item.central.nombre }} </td>
...
export default {
  ...
  methods: {
    async getTable() {
      try {
        let res = await axios.get("api/sindicato")
        this.table = res.data;
      } catch (e) {
        console.log(e);
      }
    ...
    async removeItem(item) {
      this.bus.$emit("openDialogRemove", `api/sindicato/${item.id}`);      
    },
  }
};
</script>
