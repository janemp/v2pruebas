      ...
      <v-card-text>
        ...
        <v-form ref="form" v-model="valid" lazy-validation> 
          ...
          <v-layout row wrap>
            ...
            <v-flex xs4>
              <v-autocomplete                      
                v-model="selectedPersona.departamento_extension_id"
                :items="departamentos"
                item-text="sigla"
                item-value="id"
                label="Extension"
...
export default {
  ...
  mounted() {
    // this.bus.$on("openDialogRetencionPersona", item => {
    //   this.dialog = true
  ...
  methods: {    
    ...
    async save() {
      try {
        if (this.$refs.form.validate()) {
          ...
          this.selectedPersona.user_id = this.$store.getters["auth/user"].id 
          let res = await axios.post("api/persona", this.selectedPersona)
          this.selectedItem.persona_id = res.data.id
          await axios.post("api/retencion", this.selectedItem)
    ...
    async verificarCarnet() {
      let res = await axios.get("api/persona/fill/"+ JSON.stringify({'carnet_identidad': this.selectedPersona.carnet_identidad}))
