  ...
  methods: {    
      ...
      try {
        if (this.$refs.form1.validate()) {         
          ...
          if (this.selectedIndex != -1) {
            await axios.put("api/devolucion/"+this.selectedItem.id, this.selectedItem)
          } else {
    ...
    async getResolucionAdministrativa(){
      ...
      if(res.data[0].tipo_resolucion_id == 5){        
        let res1 = await axios.get("api/devolucion/fill/" + JSON.stringify({resolucion_administrativa_id: this.selectedItem.resolucion_administrativa_id}))
