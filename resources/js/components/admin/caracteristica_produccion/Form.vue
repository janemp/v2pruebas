      ...
      <v-card-text>        
        <v-stepper v-model="step" vertical>
          <v-stepper-header>
            ...
            <v-divider></v-divider>
            <v-stepper-step step="2" :complete="step > 2">Parcela</v-stepper-step>
          </v-stepper-header>
          <v-stepper-items>
            <v-stepper-content step="1">
              ...
              <v-card flat color="blue-grey lighten-5" v-if="persona !=  null">
                <v-card-text >
                  <v-layout row wrap>
                    <v-flex xs3>
                      <v-avatar
                        size="150px">
                        <img v-bind:src="persona.fotografia">
                      </v-avatar>
                    </v-flex>
                    <v-flex xs9>
                      <table>
                        <tr>
                          ...
                          <td class="text-xs-left"> {{ fullName(persona) }} </td>
                        </tr>                  
                        <tr>
                          <th class="text-xs-left">Carnet de identidad: </th>
                          <td class="text-xs-left"> {{ persona.carnet_identidad + ' ' + persona.departamento_extension.sigla }} </td>
                        </tr>
                        <tr>
                          <th class="text-xs-left">Estado civil: </th>
                          <td class="text-xs-left"> {{ persona.estado_civil }} </td>
                        </tr>
                        <tr>
                          <th class="text-xs-left">Género: </th>
                          <td class="text-xs-left"> {{ persona.genero == 'M' ? 'Masculino' : 'Femenino' }} </td>
                        </tr>
                      </table>
                    </v-flex>
                  </v-layout>
              ...
              <v-form ref="form2" v-model="valid" lazy-validation>
                <v-layout wrap>
                  <v-flex xs12 sm6 md6>
                    ...
                    <v-autocomplete                      
                      v-model="selectedItem.comunidad_id"
                      :items="comunidades"
                      item-text="nombre"
                      item-value="id"
                      label="Comunidad"
                    ...
                    <v-text-field
                      ...
                      label="Longitud"
                      type="number"
                      :rules="[v => !!v || 'Requerido', v => /^-?([1-8]?[1-9]|[1-9]0)\.{1}\d{1,6}/.test(v) || 'longitud incorrecta']"
                      hint="<span class='blue--text'>*Requerido</span>" persistent-hint
                    ></v-text-field>
                    <v-text-field
                      v-model="selectedItem.hectareas"
                      label="Hectareas"
                      type="number"
                      :rules="[v => !!v || 'Requerido']"
                      hint="<span class='blue--text'>*Requerido</span>" persistent-hint
...
export default {
  ...
  watch: {
    begin_date(val){
      this.selectedItem.fecha_parcela = val
  ...
  methods: {    
      ...
      try {
        ...
        if (this.$refs.form2.validate()) {
          if (this.selectedIndex != -1) {
            ...
            this.selectedItem.personas = [this.persona.id]
    ...
    async getPersonas() {
      ...
      let res2 = await axios.get("api/persona/fill/"+JSON.stringify({'tipo_persona_id': 2}))
      this.temp = res.data.concat(res2.data)
      for(var temp of this.temp){
        if(temp.poder.length == 0){
          this.personas.push(temp)
        }
      }
    },
    async getPersona() {
    ...
    async getComunidades(){
      let res = await axios.get("api/comunidad")
      this.comunidades = res.data
    },
    async getRegionales(){
      let res = await axios.get("api/regional")
      this.regionales = res.data
    },
