        ...
        <v-form ref="form" v-model="valid" lazy-validation>
          <v-layout wrap>
            <v-flex xs12 sm6 md6>
              ...
              <v-card-text>                
                <v-text-field
                  v-model="selectedItem.display_name"
                  label="Display Name"
                  :rules="[v => !!v || 'Requerido', v => (v && v.length <= 50) || 'No mayor a 50 caracteres']"
                  hint="<span class='blue--text'>*Requerido</span>" persistent-hint
                ></v-text-field>
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/permission/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/permission", this.selectedItem)
          }
