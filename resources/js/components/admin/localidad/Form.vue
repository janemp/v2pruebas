...
export default {
  ...
  methods: {
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/localidad/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/localidad", this.selectedItem)
          }
    ...
    async nameValidate(){
      var params = {'nombre': this.selectedItem.nombre}
      let res = await axios.get('api/localidad/fill/'+JSON.stringify(params))
