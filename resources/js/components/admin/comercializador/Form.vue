  ...
  <v-dialog persistent v-model="dialog" max-width="900px" @keydown.esc="close">
    ...
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-btn v-if="step!=2 && sustituir == false" color="info" small @click.native="nextStep()">Siguiente</v-btn>
        <v-btn v-if="sustituir == true && step != 3" color="info" small @click.native="nextStep()">Siguiente</v-btn>
...
export default {
  ...
  mounted() {
    this.bus.$on("openDialog", item => {
      ...
      if(this.sustituir){
        ...
        this.getProvincias()
        this.getProvinciasProduccion()
        this.getMunicipiosProduccion()
      }
    });
    this.getPaises()
    ...
    this.getDepartamentosProduccion()
    this.getProvinciasProduccion()
    this.getMunicipiosProduccion()
  ...
  methods: {    
      ...
      try {
        ...
        if (this.$refs.form2.validate() && this.sustituir == false) {
          ...
          if (this.selectedIndex != -1) {            
            ...
            this.selectedItem.tipo_persona_id = 4
    ...
    //Produccion
    async getDepartamentosProduccion() {
      let res = await axios.get("api/departamento")
      this.departamentosProduccion = res.data
    },    
    async getProvinciasProduccion() {
      let res = await axios.get("api/provincia/fill/" + JSON.stringify({'departamento_id': this.selectedItem.departamento_produccion_id}))
      this.provinciasProduccion = res.data
    },
    async getMunicipiosProduccion() {
      let res = await axios.get("api/municipio/fill/" + JSON.stringify({'provincia_id': this.selectedItem.provincia_produccion_id}))
      this.municipiosProduccion = res.data
    },
    // async getRegiones() {
    ...
    async getSustituciones() {
      ...
      console.log(this.sustituciones)
    },
