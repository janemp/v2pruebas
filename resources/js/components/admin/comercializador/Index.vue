<template>
  <v-container fluid>
    <v-toolbar>
        <v-toolbar-title>Comercializadores al detalle</v-toolbar-title>
        <v-spacer></v-spacer>   
        <v-divider
          class="mx-2"
          inset
          ...
          <tr>
            ...
            <td class="justify-center layout">
              <v-tooltip top v-if="props.item.sustitucion_id==null">
                ...
                <v-tooltip top v-if="props.item.sustitucion_id==null && props.item.cesaciones.length==0">
                  <v-btn slot="activator" flat icon color="indigo" @click="cardItem(props.item)">
                    <v-icon>credit_card</v-icon>
                  </v-btn>
                  <span>Carnet de productor</span>
                </v-tooltip>               
                <v-tooltip top v-if="props.item.tipo_persona_id==4 && props.item.sustitucion_id==null && props.item.cesaciones.length==0">
...
export default {
  ...
  methods: {
    async getTable() {
      try {
        let res = await axios.get("api/persona/fill/" + JSON.stringify({tipo_persona_id: 4}))
        this.table = res.data
      } catch (e) {
        console.log(e)
      }
    ...
    async removeItem(item) {
      let res = await axios.get("api/hoja_ruta/fill/" + JSON.stringify({persona_id: item.id}));
      console.log(res.data)
      if(res.data.length > 0) {
        this.$toast.info('Este comercializador tiene hojas de ruta.')
      } else {
