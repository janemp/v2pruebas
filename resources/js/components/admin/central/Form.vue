...
export default {
  ...
  methods: {
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/central/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/central", this.selectedItem)
          }
    ...
    codevalidate(){
      axios.get('/api/central/codigo/'+this.selectedItem.codigo).then(response =>{
