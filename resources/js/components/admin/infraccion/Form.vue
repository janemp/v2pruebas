        ...
        <v-form ref="form" v-model="valid" lazy-validation>
          <v-layout wrap>
            ...
            <v-flex xs12 sm6 md6>
              <v-card-text>                
                <v-text-field
                  v-model="selectedItem.severidad"
                  label="Severidad"
                  :rules="[v => !!v || 'Requerido', v => (v && v.length <= 10) || 'No mayor a 10 caracteres']"
                  hint="<span class='blue--text'>*Requerido</span>" persistent-hint
                ></v-text-field>              
              </v-card-text>
            </v-flex>
...
export default {
  ...
  methods: {
      ...
      try {
        if (this.$refs.form.validate()) {
          if (this.selectedIndex != -1) {
            await axios.put("api/infraccion/"+this.selectedItem.id, this.selectedItem)
          } else {
            await axios.post("api/infraccion", this.selectedItem)
          }
    ...
    codevalidate(){
      axios.get('/api/infraccion/codigo/'+this.selectedItem.codigo).then(response =>{
